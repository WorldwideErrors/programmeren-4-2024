...
const CLEAR_DB = CLEAR_MEAL_TABLE + CLEAR_MEAL_PARTICIPANTS_TABLE + CLEAR_USERS_TABLE
...
const INSERT_MEAL =
    ...
    '(1, 1, 0, 0, 0, "2022-04-13 11:24:46", 3, 6.30, "https://www.google.com/url?sa=i&url=https", 1, "Pizza Tonno", "De lekkerste pizza van Rome!"),' +
    ...
    '(3, 1, 0, 0, 1, "2023-04-21 18:47:37", 1, 9.50, "https://www.google.com/url?sa=i&url=https%3A%JiB_pAAABAE", 2, "Hamburger", "Heb jij ook zin in een heerlijke hamburger met friet?");'

const INSERT_MEAL_PARTICIPANTS =
...
describe('UC-401 Aanmelden voor maaltijd', () => {
    beforeEach((done) => {
        ...
        db.getConnection(function (err, connection) {
            ...
            // Use the connection
            connection.query(
                CLEAR_DB + INSERT_USER + INSERT_MEAL + INSERT_MEAL_PARTICIPANTS,
    ...
    it('TC-401-1 Niet ingelogd', (done) => {
        chai.request(server)
            .post(endpointToTest + '/1/participate')
            .end((err, res) => {
    ...
    it('TC-401-3 Succesvol aangemeld', (done) => {
        ...
        chai.request(server)
            ...
            .end((err, res) => {
                ...
                chai.expect(res.body)
                    .to.have.property('message')
                    .equals('Nieuwe deelname toegevoegd.');
                chai.expect(res.body)
                    .to.have.property('data')
    ...
    it('TC-401-4 Maximumaantal aanmeldingen is bereikt.', (done) => {
        ...
        chai.request(server)
            .post(endpointToTest + '/3/participate')
            .set('Authorization', `Bearer ${token}`)
            .end((err, res) => {
                chai.expect(res).to.have.status(200);
                chai.expect(res.body).to.be.a('object');
                ...
                chai.expect(res.body)
                    .to.have.property('message')
                    .equals('Maximumaantal aanmeldingen is bereikt.');
                done();
            });
    });
});